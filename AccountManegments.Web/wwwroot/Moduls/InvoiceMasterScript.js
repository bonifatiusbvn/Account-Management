    ...
    if ($("#CreateInvoiceForm").valid()) {
        ...
        if ($('#addNewlink tr').length >= 1 && $('#dvShippingAdd .row.ac-invoice-shippingadd').length >= 1) {
            ...
            var PORequest = {
                ...
                Poid: $("#txtPoId").val(),
                Date: $("#orderdate").val(),
                FromSupplierId: $("#textSupplierName").val(),
                ToCompanyId: $("#textcompanyName").val(),
                TotalAmount: $("#cart-total").val(),
                TotalGstamount: $("#totalgst").val(),
                BillingAddress: $("#textcompanybillingaddressDetails").val(),
...
$(document).ready(function () {
    ...
    $("#CreateInvoiceForm").validate({
        ...
        messages: {
            ...
            textcompanybillingaddressDetails: "Enter Billing Address",
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response) {
                ...
                $('#textcompanybillingaddressDetails').val(response.fullAddress);
            } else {
                console.log('Empty response received.');
            }
        },
...
function checkandDisableAddButton() {
    ...
    if ($('#addNewlink tr').length > 1) {
        $('.add-addresses').prop('disabled', true);
    } else {
